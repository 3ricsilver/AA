...
void die(char *s)// son truc pour monterer un message d'erreur en gros erreur message
{
    perror(s);
    exit(1);
}

int main(int argc,char *argv[])// pour créer
{
 int rc ;
 int Desc ;
 ...
 if ( argc!=3)
 {
  printf("ser ser port cli (frere respecte le format)\n") ;
  exit(1) ;
 }
 ...
 Desc=creer_socket(SOCK_DGRAM,&IpSocket,PortSocket,&sthis);//créer le socket puis on verif ces valeurs
 ...
 printf("en anteente d'une info du client\n");
...
//c'est une recherche bro je vais pas expliqué 
   ...
   printf ("\n\t-- Insertion --\n");
    ...
    while ( fgets(Buffer,sizeof Buffer,fp),!feof(fp) )
    {
      ...
      if((strcmp(B1, "file")) == 0)
      {
         ...
         strcpy(pZone->BR,"Truong Datolli") ;
