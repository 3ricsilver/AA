...
const char* typeStrings[] = { "", "NS", "A", "PTR", "BR" };

// Define BR as the length of typeStrings - 1
#define BR (sizeof(typeStrings) / sizeof(typeStrings[0])) - 1
...
const char* enumToString(enum TypeIN type) {
    static const char* typeStrings[] = { "", "NS", "A", "PTR", "BR" };
    return typeStrings[type];
}


void listDBBR (sll_s * p_l)
{
   int i;
   int size ;
...
int LectureDBBR(char *Named, sll_s *p_l)
{
...
char *RechercheDBBR(char *Nom,int Type,sll_s * p_l)
{
   int i;
   int size ;
   struct EntreeZone *pEntreeZone ;
...
int main (void)
{
   ...
   sll_s *p_sll = NULL;
   p_sll = sll_new () ;
   LectureDBBR("./domaine/BeguinRadoux.zone",p_sll) ;
   printf("###################################################\n") ;
   listDBBR(p_sll) ;
   ...
   while(1)
   {
      ...
      pchaine= RechercheDBBR(Entree, Type, p_sll ) ;
