  ...
  while(1)
  {
    ...
    switch(m.requete)
    {
      ...
      fprintf(stderr,"notre pid:(%d)->(%d) avec comme data1 %s avec comme requete %d \n",m.expediteur,m.type,m.data1,m.requete);
       ...
       // i=0,i2=0,i3=0,k=0;//eric du futur je viens de decouvrir que tu peux pas créer une valeur dans un case genre int chadric;


       // for(i=0;i<6;i++)// on trouve le pid de la personne dont on  accepter un message
       // {
       //    if(strcmp(m.data1,tab->connexions[i].nom)==0)
       //    {
            
       //      VariableTemp=tab->connexions[i].pidFenetre;

       //      for(i3=0;i3<6;i3++)// on trouve l'endroid pid de la personne qui a envoyer la requete
       //      {
       //        if(m.expediteur==tab->connexions[i3].pidFenetre)
       //        {
       //          j=i3;
       //          i3=6;
       ...
       //          for(i2=0;i2<5;i++)
       //          {
       //            if(tab->connexions[j].autres[i2] != 0)
       //            {
       //              tab->connexions[j].autres[i2] =VariableTemp;
       //              i2=5;
       //            }
       //          }
      ...
      for(k=0;k<6;k++)
      {
        ...
        //pourquoi les 3 autres manieres que j'ai fait ne marche pas
